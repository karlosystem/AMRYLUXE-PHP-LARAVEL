            ...
            <div class="col-md-12">
                <div class="customers__area bg-style mb-30">
                    ...
                    <div class="item-title">
                        <div class="col-xs-6">
                            <a href="{{ route('admin.categoria.create') }}" class="btn btn-md btn-info">Agregar
                                Categoria</a>
                        </div>
                    </div>
                                ...
                                <tbody>
                                    @foreach ($categorias as $data)
                                        <tr>
                                            ...
                                            <td>{{ $data->nombre }}</td>
                                            <td>{{ $data->slug }}</td>
                                            <td>{{ $data->tipo->p_cat_name }}</td>
                                            <td>
                                                @if ($data->imagen)
                                                    <img src="{{ asset('front/assets/images/categorias/' . $data->imagen) }}"
                                                        width="50">
                                                @else
                                            ...
                                            <td class="text-center">
                                                <div class="d-flex justify-content-center gap-2">
                                                    {{-- Botón Editar --}}
                                                    <a href="{{ route('admin.categoria.edit', $data->id) }}"
                                                        class="btn btn-sm btn-warning" title="Editar">
                                                        <i class="fas fa-edit"></i>
                                                    </a>

                                                    {{-- Botón Eliminar --}}
                                                    <form action="{{ route('admin.categoria.destroy', $data->id) }}"
                                                        method="POST"
